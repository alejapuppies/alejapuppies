...
export default function UserProfile(props){
    ...
    const userDataLog = props.user;
    const [user, setUser] = useState(initialStateUser);

    const showFullInfo = () =>{
        var usersRef = firebase.database().ref("users");
        usersRef.on("child_added", function(snapshot) {
            if(snapshot.child("email").val() == userDataLog.email){
        ...
        <div className="mt-5 container">
            ...
            <section className="mt-5">
                <div className="row w-100 mx-auto">
                    <button className="btn btn-primary mx-auto" onClick={() => showFullInfo()}>Perfil</button>
                    <button className="btn btn-primary mx-auto">Mascotas</button>
            ...
            <div className="row w-100 mx-auto">
                <div className="mx-auto container m-3 card-shadow col-12">
                    ...
                    <p className="text-black"><strong>Direccion: </strong>{user.adress}</p>
